...
def Density_test_batch(test_name, null_samples, alter_samples, null_model, alter_model=None, bootstrap_approx=True,
                       num_bootstrap=1000, alpha=0.05, training_args=None):
    ...
    if not training_args:
        ...
        bootstrap_null_samples = m_out_n_bootstrap(test_name, num_bootstrap, num_samples, sub_samples, null_samples,
                                                   null_model, alter_model)
        start_time = time.time()
        for i in range(num_tests):
            test_statistic, pvalue = Density_test(test_name, alter_samples[i], bootstrap_null_samples, null_model,
                                                  alter_model, bootstrap_approx)
        ...
        print('mse between the null and the true alternative',
              verfy_mse(null_model.W, training_args['params_ptb'][training_args['alter_ptb_name']]))
        print('mse between the null and the estimated alternative', verfy_mse(null_model.W, alter_model.W))
        print('mse between the true alternative and the estimated alternative',
              verfy_mse(training_args['params_ptb'][training_args['alter_ptb_name']], alter_model.W))
