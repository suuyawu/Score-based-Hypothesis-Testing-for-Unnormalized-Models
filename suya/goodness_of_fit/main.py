...
def simulation(eval_name, dist_name, test_names, params, alter_ptb_name, num_tests):
    if eval_name == 'alternative_shift':
        if dist_name == '1dgmm':
            eval_levels = alter_ptd_std = np.array(
                [0, 0.1, 0.15, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.7, 1, 1.5, 2, 2.5, 3])
        if dist_name == 'gbrbm':
            eval_levels = alter_ptd_std = np.array(
                [0, 0.005, 0.007, 0.009, 0.01, 0.011, 0.012, 0.013, 0.014, 0.016, 0.018, 0.02, 0.025, 0.03, 0.035])
    ...
    if eval_name == 'noisy_data':
        eval_levels = data_ptd_std = np.array(
            [0, 0.005, 0.007, 0.009, 0.012, 0.03, 0.05, 0.07, 0.09, 0.1, 0.2, 0.5, 0.7, 1])
    ...
    for i, (num_samples, eps_a, eps_d) in enumerate(zip(sample_size, alter_ptd_std, data_ptd_std)):
        print(
            '[Evaluation Setup] Sample size: {}; Alternative model perturbation level: {}; Data perturbation level {}'.format(
                num_samples, eps_a, eps_d))
        if dist_name == '1dgmm':
            ...
            params_ptb[alter_ptb_name] = params_ptb[alter_ptb_name] + eps_a * torch.randn_like(
                params_ptb[alter_ptb_name])
        ...
        if 'LikelihoodRatio Chi2' in test_names:
            result_lrt = Density_test_batch('LikelihoodRatio Chi2', null_samples, alter_samples, model0,
                                            alter_model=model1, bootstrap_approx=False)
        ...
        if 'LikelihoodRatio Bootstrap' in test_names:
            result_lrt = Density_test_batch('LikelihoodRatio Bootstrap', null_samples, alter_samples, model0,
                                            alter_model=model1, bootstrap_approx=True)
        ...
        if 'HScore' in test_names:
            result_hscore = Density_test_batch('HScore', null_samples, alter_samples, model0, alter_model=model1,
