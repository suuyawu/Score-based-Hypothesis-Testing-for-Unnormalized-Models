...
def simulation(eval_name, dist_name, test_names, params, alter_ptb_name, num_tests):
    if eval_name == 'alternative_shift':
        ...
        if dist_name == 'gbrbm':
            eval_levels = alter_ptd_std = np.array([0, 0.005, 0.007, 0.009, 0.012, 0.03, 0.05, 0.1])
    ...
    for i, (num_samples, eps_a, eps_d) in enumerate(zip(sample_size, alter_ptd_std, data_ptd_std)):
        ...
        if dist_name == '1dgmm':
            ...
            model1 = GaussianMixture(params_ptb)
        elif dist_name == 'gbrbm':
            model0 = GaussBernRBM(params)
            params_ptb= params.copy()
            params_ptb[alter_ptb_name] = params_ptb[alter_ptb_name]+eps_a*torch.randn_like(params_ptb[alter_ptb_name])
            model1 = GaussBernRBM(params_ptb)
        
        null_samples = model0.sample(num_samples*10)
        alter_samples = model1.sample(num_samples*num_tests)
        ...
        if 'LikelihoodRatio Chi2' in test_names:
            result_lrt = Density_test_batch('LikelihoodRatio Chi2', null_samples, alter_samples, model0, model1, bootstrap_approx=False)
        ...
        if 'LikelihoodRatio Bootstrap' in test_names:
            result_lrt = Density_test_batch('LikelihoodRatio Bootstrap', null_samples, alter_samples, model0, model1, bootstrap_approx=True)
        ...
        if 'HScore' in test_names:
            result_hscore = Density_test_batch('HScore', null_samples, alter_samples, model0, model1)
